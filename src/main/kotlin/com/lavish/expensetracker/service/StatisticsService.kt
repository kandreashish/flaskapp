...
class StatisticsService(
    ...
    fun getUserStats(userId: String, period: Period): UserStats {
        ...
        val categoryData = expenseRepository.getCategoryWiseExpensesForUser(userId, startDate, endDate)
        val categoryWiseExpenses = processCategoryData(categoryData, totalExpenses)

        // Get currency-wise expenses
        val currencyData = expenseRepository.getCurrencyWiseExpensesForUser(userId, startDate, endDate)
        val currencyWiseExpenses = processCurrencyData(currencyData)

        // Get monthly trend
        val monthlyData = expenseRepository.getMonthlyExpensesForUser(userId, startDate, endDate)
    ...
    fun getFamilyStats(familyId: String): FamilyStats {
        val period = Period.CURRENT_MONTH // Default to current month for family stats
        val dateRange = getDateRange(period)
        val startDate = dateRange.first
        val endDate = dateRange.second
        ...
        val totalCount = totalAndCount.getExpenseCount().toInt()
    ...
    private fun processCategoryData(categoryData: List<Array<Any>>, totalExpenses: Double): List<CategoryExpense> {
        return categoryData.map { row ->
            ...
            val amount = (row[1] as? BigDecimal)?.toDouble() ?: (row[1] as Double)
    ...
    private fun processCurrencyData(currencyData: List<Array<Any>>): List<CurrencyExpense> {
        return currencyData.map { row ->
            ...
            val totalAmount = (row[1] as? BigDecimal)?.toDouble() ?: (row[1] as Double)
            val count = (row[2] as Long).toInt()
    ...
    private fun processFamilyMemberData(memberData: List<Array<Any>>, totalFamilyExpenses: Double): List<FamilyMemberStats> {
        ...
        return userMap.map { (userId, rows) ->
            ...
            val totalUserExpenses = rows.sumOf { (it[1] as? BigDecimal)?.toDouble() ?: (it[1] as Double) }
